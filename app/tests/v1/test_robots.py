...
def test_create_robot_endpoint():
  ...
  assert data["robot_new"]["name"] == robot_new["name"]
...
def test_read_robots_endpoint():
  ...
  data = response.json()
  assert data["status"] == "ok"
  assert len(data["robots"]) == 1  # Assuming we have one robot created in the previous test.

def test_update_robot_endpoint():
  data = client.get("/api/v1/robots").json()
  robot_id = data["robots"][0]["id"]  # Get the ID of the first robot (To update it).
  ...
  data = response.json()
  assert data["status"] == "ok"
  assert data["robot_updated"]["id"] == robot_id, "Robot ID should not change"
  assert data["robot_updated"]["name"] == robot_updated_config["name"]
